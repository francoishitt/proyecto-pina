  ...
  const onSubmit = async (data: CursoFormData) => {
    ...
    if (modo === "crear") {
      if (!data.pdf || data.pdf.length === 0) {
        toast.error("El archivo PDF es obligatorio para crear un material");
        return;
      }
    }
    ...
    try {
      ...
      if (res.success) {
        toast.success(modo === "crear" ? "Se creó correctamente el material" : "Se actualizó correctamente el material");
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl border border-slate-300 p-6 space-y-6">
      <h2 className="text-xl font-semibold text-slate-800">
        {modo === "crear" ? "Nuevo material" : "Editar material"}
      </h2>
