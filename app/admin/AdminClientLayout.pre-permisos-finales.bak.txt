  ...
  if (isSidebarOpen === null) {
    return (
      <div className="flex h-screen bg-slate-100 items-center justify-center">
        <div className="animate-pulse text-slate-500 font-medium">Cargando panel...</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-slate-100 text-slate-800 overflow-hidden">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {links.map((link) => {
            ...
            return (
              ...
              >
                <Icono className={`w-4 h-4 ${activo ? "text-white" : "text-orange-500"}`} />
                {link.label}
              </Link>
            );
          })}
      ...
      >
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-xl shadow-lg py-1 z-50">
                  <div className="px-4 py-3 border-b border-slate-100">
                    <p className="text-sm font-medium text-slate-800 truncate">
                      {`${usuarioInicial.nombre} ${usuarioInicial.apellidos}`.trim() || usuarioInicial.email}
                    </p>
                    <p className="text-xs text-slate-500 truncate">{usuarioInicial.email}</p>
                    <p className="text-[10px] font-bold text-orange-600 mt-1">{usuarioInicial.rol}</p>
                  </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 bg-slate-50">{children}</main>
      </div>
    </div>
  );
}
