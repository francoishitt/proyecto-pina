...
import { exigirAdmin, exigirUsuarioPanel, mensajeErrorPermisos } from "@/lib/auth";
...
async function uploadFile(file: File, tipoLog: "PDF" | "Portada"): Promise<string> {
  const ext = file.name.split(".").pop()?.toLowerCase();

  if (tipoLog === "PDF") {
    if (file.size > 20 * 1024 * 1024) throw new Error("El PDF no puede superar 20 MB.");
    if (file.type !== "application/pdf" || ext !== "pdf") throw new Error("Solo se permiten archivos PDF.");
  } else {
    const extensiones = ["jpg", "jpeg", "png", "webp"];
    const tipos = ["image/jpeg", "image/png", "image/webp"];
    if (file.size > 5 * 1024 * 1024) throw new Error("La portada no puede superar 5 MB.");
    if (!ext || !extensiones.includes(ext) || !tipos.includes(file.type)) {
...
export async function crearCurso(formData: FormData) {
  try {
    ...
    return { success: false, error: error instanceof Error ? error.message : "Error al crear el material." };
  }
}
...
export async function actualizarCurso(id: string, formData: FormData) {
  try {
    ...
    return { success: false, error: error instanceof Error ? error.message : "Error al actualizar el material." };
  }
}
